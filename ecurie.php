...
$ecurie = isset($_GET['ecurie']) ? $_GET['ecurie'] : '';

try {
    // Préparer et exécuter la requête sécurisée
    $sql = "SELECT * FROM produits WHERE ecurie = :ecurie";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':ecurie', $ecurie, PDO::PARAM_STR);
    $stmt->execute();
...
<head>
    ...
    <title>Produits de <?php echo htmlspecialchars($ecurie); ?></title>
    <link rel="icon" type="image/png" href="assets/favicon.png">
    <link rel="stylesheet" href="core/style/style.css">
</head>
<body>
    <h1 class="title-pilote">Achetez des articles officiels de F1 <?php echo htmlspecialchars($ecurie); ?></h1>
    <div class="produits">
    <?php if (count($produits) > 0) { ?>
        ...
        <?php } ?>
    <?php } else { ?>
        <p>Aucun produit trouvé pour cette écurie.</p>
    <?php } ?>
</div>
